...
/**
 * Ingredient Dictionary Seed — Batch 40
 * แคปไซซิน / Capsaicin (Topical analgesic)
 *
 * Single ingredient, 16 synonyms (Thai: 6 / English+scientific: 10).
 *
 * NOTES:
 *   Canonical "capsaicin" — the active compound, not the plant (Capsicum spp.).
 *   "Capsicum" plant names NOT included as synonyms: Capsicum annuum / C.
 *   frutescens appear on food/nutritional and chili supplement labels in addition
 *   to topical products → false-match risk. Topical-specific forms kept.
 *
 *   "Capsicum oleoresin" / "oleoresin capsicum" included: these are the bulk
 *   extract forms used in topical formulations AND OC self-defence sprays. In
 *   Thai pharmacy context, products containing these terms are almost always
 *   topical analgesics (not OC spray), so the risk of cross-match is low.
 *
 *   Product-form variants added beyond user list:
 *     - แคปไซซินแผ่นแปะ / แผ่นแปะแคปไซซิน / capsaicin patch — patch is sold
 *       in Thai pharmacy and product names often include the Thai form.
 *     - แคปไซซินขี้ผึ้ง — Thai for "capsaicin ointment".
 *
 *   preferredCategory "ยาทาภายนอก" — topical OTC drug category.
 *
 * Default --dry-run; --commit persists.
 *   node scripts/seed_ingredient_dictionary_batch40.js [--dry-run|--commit] [--db-url <url>]
 */
...
const SOURCE = "seed_dictionary_batch_40";

const INGREDIENTS = [
  {
    canonical: "capsaicin",
    display: "Capsaicin / Topical Analgesic (แคปไซซิน / Capsaicin)",
    drugClass: "Topical analgesic / Counterirritant (Capsaicin / Capsicum)",
    indications: [
      "Topical analgesia / Muscle pain (OTC)",
      "Joint pain / Arthritis (topical)",
      "Neuropathic pain (topical)",
    ],
    preferredCategory: "ยาทาภายนอก",
    synonyms: [
      // ── Thai names (compound + product-form variants) ─────────────────────
      "แคปไซซิน",
      "แคปไซซินครีม",
      "แคปไซซินเจล",
      "แคปไซซินขี้ผึ้ง",
    ...
    for (const def of INGREDIENTS) {
      ...
      if (resolvedCategory) {
        const ruleInserted = await insertCategoryRuleIfMissing(client, {
          ...
          note: `Batch 40 rule: ${def.canonical} -> ${resolvedCategory}`,
        });
...
function printSummary(stats) {
  const lines = [];
  lines.push("==================================================");
  lines.push(` INGREDIENT DICTIONARY SEED — BATCH 40 (แคปไซซิน / Capsaicin)  [${stats.mode.toUpperCase()}]`);
  lines.push("==================================================");
...
async function main() {
  ...
  if (args.help) { console.log("node scripts/seed_ingredient_dictionary_batch40.js [--dry-run|--commit] [--db-url <url>]"); return; }
...
if (require.main === module) {
  main().catch((err) => { console.error(`Batch 40 seed failed: ${err.message}`); process.exitCode = 1; });
}
