...
/**
 * Ingredient Dictionary Seed — Batch 29 (เสลดพังพอน — both species).
 *
 * TWO SEPARATE INGREDIENTS — must NEVER be merged:
 *
 *   เสลดพังพอนตัวเมีย / พญายอ  = Clinacanthus nutans
 *     Primary use: antiviral (antiherpetic), immunomodulator, anti-inflammatory skin
 *     Active parts: leaves (ใบ) and stems
 *
 *   เสลดพังพอนตัวผู้ / พิมเสนต้น = Barleria lupulina
 *     Primary use: anti-inflammatory, analgesic (muscle/joint pain)
 *     Active parts: leaves and roots
 *
 * CRITICAL AMBIGUITY (encoded in MODELING_NOTES):
 *   "เสลดพังพอน" alone (without ตัวเมีย / ตัวผู้ qualifier) is intentionally
 *   excluded from BOTH entries — it is ambiguous and could refer to either species.
 *   If a product label says only "เสลดพังพอน" without qualifier, mark for human review.
 *
 * Trimmed from user's list:
 *   "Clinacanthus nutans (Burm.f.) Lindau" — "(Burm.f.) Lindau" = author notation
 *   "Barleria lupulina Lindl."             — "Lindl." = author abbreviation
 *
 * Added vs user's list (C. nutans / ตัวเมีย):
 *   "สมุนไพรพญายอ"    — herbal-prefix form used on Thai supplement labels
 *   "ใบพญายอ"        — leaf-specific form; the leaf is the primary medicinal part
 *   "sabah snake grass"         — very common English name; appears on bilingual labels
 *   "sabah snake grass extract" — extract form of the above
 *   "phaya yo"                  — romanized Thai name on export/international labels
 *
 * Added vs user's list (B. lupulina / ตัวผู้):
 *   "สมุนไพรพิมเสนต้น" — herbal-prefix form used on Thai supplement labels
 *
 * C. nutans synonyms (11): Thai (6) + English/scientific (5)
 * B. lupulina synonyms (9): Thai (5) + English/scientific (4)
 *
 * preferredCategory = "ยาสมุนไพร" for both — skipped gracefully if not confirmed in DB.
 *
 * Default --dry-run; --commit persists.
 *   node scripts/seed_ingredient_dictionary_batch29.js [--dry-run|--commit] [--db-url <url>]
 */
...
const SOURCE = "seed_dictionary_batch_29";
const PREFERRED_CATEGORY = "ยาสมุนไพร";

const MODELING_NOTES = [
  "Batch 29 = เสลดพังพอน — TWO separate species: ตัวเมีย (Clinacanthus nutans) and ตัวผู้ (Barleria lupulina).",
  "CRITICAL: These are DIFFERENT plants with different active compounds and different indications. Do NOT merge.",
  "'เสลดพังพอน' alone (no ตัวเมีย/ตัวผู้) EXCLUDED from both entries: ambiguous — requires human review on product audit.",
  "C. nutans (ตัวเมีย / พญายอ): antiviral (antiherpetic), anti-inflammatory skin, immunomodulator; leaf is primary medicinal part.",
  "B. lupulina (ตัวผู้ / พิมเสนต้น): anti-inflammatory, analgesic for muscle/joint pain.",
  "'Clinacanthus nutans (Burm.f.) Lindau' trimmed: '(Burm.f.) Lindau' = botanical author notation, not a label token.",
  "'Barleria lupulina Lindl.' trimmed: 'Lindl.' = botanical author abbreviation, not a label token.",
  "'sabah snake grass' added for C. nutans: common English name on bilingual Thai export labels.",
  "'phaya yo' added for C. nutans: romanized Thai name found on export/international supplement labels.",
  "'ใบพญายอ' added for C. nutans: leaf-specific form; the leaf is the primary part used medicinally.",
  "preferredCategory 'ยาสมุนไพร' for both — skipped gracefully if not yet confirmed in product_category_states.",
];

const INGREDIENTS = [
  {
    canonical: "clinacanthus nutans",
    display: "Sabah Snake Grass / Phaya Yo (เสลดพังพอนตัวเมีย / พญายอ)",
    drugClass: "Herbal antiviral / Immunomodulator (Clinacanthus nutans)",
    indications: [
      "Antiviral / Antiherpetic (herbal)",
      "Anti-inflammatory (herbal)",
      "Wound healing / Skin soothing (herbal)",
    ],
  ...
  {
    canonical: "barleria lupulina",
    display: "Hop-Headed Barleria / Phimsen Ton (เสลดพังพอนตัวผู้ / พิมเสนต้น)",
    drugClass: "Herbal anti-inflammatory / Analgesic (Barleria lupulina)",
    indications: [
      "Anti-inflammatory (herbal)",
    ...
    synonyms: [
      ...
      "พิมเสนต้น",
      "สารสกัดพิมเสนต้น",
      "สารสกัดเสลดพังพอนตัวผู้",
      "สมุนไพรพิมเสนต้น",
      // ── Botanical / scientific names ──────────────────────────────────────
      "barleria lupulina",
      "barleria lupulina extract",
      // ── English common names ──────────────────────────────────────────────
      "hop-headed barleria",
      "hop-headed barleria extract",
    ],
  },
];
    ...
    for (const def of INGREDIENTS) {
      ...
      if (resolvedCategory) {
        const ruleInserted = await insertCategoryRuleIfMissing(client, {
          ...
          note: `Batch 29 rule: ${def.display} -> ${resolvedCategory}`,
        });
...
function printSummary(stats) {
  const lines = [];
  lines.push("==================================================");
  lines.push(` INGREDIENT DICTIONARY SEED — BATCH 29 (เสลดพังพอน × 2 species)  [${stats.mode.toUpperCase()}]`);
  lines.push("==================================================");
  ...
  if (stats.uncertainCategoryMappings.length) {
    ...
    lines.push("  → Confirm 'ยาสมุนไพร' via Review Queue, then re-run --commit to attach the rules.");
  }
...
async function main() {
  ...
  if (args.help) { console.log("node scripts/seed_ingredient_dictionary_batch29.js [--dry-run|--commit] [--db-url <url>]"); return; }
...
if (require.main === module) {
  main().catch((err) => { console.error(`Batch 29 seed failed: ${err.message}`); process.exitCode = 1; });
}
