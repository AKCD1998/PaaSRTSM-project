...
/**
 * Ingredient Dictionary Seed — Batch 45
 * Branched-Chain Amino Acids (BCAA blend) — 1 collective supplement entry
 *
 * Single ingredient, 10 synonyms (Thai: 4 / English: 6).
 *
 * ── KEY MODELING DECISIONS ─────────────────────────────────────────────────
 *
 *   BCAA blend = SEPARATE entry (per batch 39 note). A product labelled
 *   "BCAA" is a leucine+isoleucine+valine blend, NOT any one individual amino
 *   acid. Do NOT add "bcaa"/"branched chain amino acids" as synonyms of
 *   leucine, isoleucine, or valine.
 *
 *   EAA terms remain separate under batch 39:
 *     "EAA", "EAAs", "essential amino acids" belong to the broader 9-amino-
 *     acid blend entry and are NOT synonyms of BCAA.
 *
 *   Component names / triplets are NOT synonyms of the blend:
 *     leucine, isoleucine, valine, "leucine isoleucine valine", and similar
 *     label lists can appear on non-BCAA amino-acid products or full EAA
 *     formulas. Those should map to the individual amino acids already seeded
 *     in batch 39, not force a blend-level match.
 *
 *   Short-code exception:
 *     "BCAA" is intentionally INCLUDED even though short abbreviations are
 *     usually risky, because it functions as the dominant product-class name
 *     on supplement labels and is materially less ambiguous than 3-letter
 *     amino-acid codes like Leu/Ile/Val.
 *
 * Total: 1 ingredient, 10 synonyms (Thai: 4 / English: 6)
 *
 * Default --dry-run; --commit persists.
 *   node scripts/seed_ingredient_dictionary_batch45.js [--dry-run|--commit] [--db-url <url>]
 */
...
const SOURCE = "seed_dictionary_batch_45";

const MODELING_NOTES = [
  "Batch 45 = BCAA blend (branched-chain amino acid blend); 1 collective entry.",
  "BCAA blend is a SEPARATE entry — label term 'BCAA' is not any one individual amino acid.",
  "  leucine / isoleucine / valine remain separate ingredient entries from batch 39.",
  "EAA terms excluded here — 'EAA/EAAs/essential amino acids' belong to the broader 9-amino-acid blend entry.",
  "Component triplets excluded — 'leucine isoleucine valine' can appear as ingredient lists, not blend names.",
  "Short-code exception: 'BCAA' included because it is the standard supplement shelf/product label term.",
];

const INGREDIENTS = [
  {
    canonical: "branched-chain amino acid blend",
    display: "Branched-Chain Amino Acids / BCAA Blend (กรดอะมิโนโซ่กิ่ง / BCAA)",
    drugClass: "Branched-chain amino acid supplement (BCAA blend: leucine + isoleucine + valine)",
    indications: [
      "Branched-chain amino acid supplementation",
      "Muscle recovery / Exercise recovery (supplement)",
      "Muscle protein synthesis support (supplement)",
      "Athletic performance / Endurance support (supplement)",
    ],
    ...
    for (const def of INGREDIENTS) {
      ...
      if (resolvedCategory) {
        const ruleInserted = await insertCategoryRuleIfMissing(client, {
          ...
          note: `Batch 45 rule: ${def.canonical} -> ${resolvedCategory}`,
        });
...
function printSummary(stats) {
  const lines = [];
  lines.push("==================================================");
  lines.push(` INGREDIENT DICTIONARY SEED — BATCH 45 (BCAA blend)  [${stats.mode.toUpperCase()}]`);
  lines.push("==================================================");
...
async function main() {
  ...
  if (args.help) { console.log("node scripts/seed_ingredient_dictionary_batch45.js [--dry-run|--commit] [--db-url <url>]"); return; }
...
if (require.main === module) {
  main().catch((err) => { console.error(`Batch 45 seed failed: ${err.message}`); process.exitCode = 1; });
}

module.exports = { parseCliArgs, seed, INGREDIENTS, MODELING_NOTES };
