...
/**
 * Ingredient Dictionary Seed — Batch 10 (Set 10: Gauze / Wound Dressing).
 *
 * Models a single broad "gauze" category-ingredient covering gauze pads, swabs,
 * rolls, bandages, non-woven gauze and sterile compresses. Like soap (batch 6)
 * and alcohol (batch 7) this is a product-type/shelf term rather than a true
 * active ingredient — seeded so the review queue can propose the wound care
 * supplies shelf for these products.
 *
 * This is the first batch with Thai synonyms. Thai synonyms are stored for
 * admin search but DO NOT drive product matching today (matcher is Latin-only;
 * normalizeLatin strips Thai script to empty / non-Latin tokens which are
 * guarded against by the backfill loadActiveSynonyms filter).
 *
 * Bare "swab" is included per user decision (2026-06-16). It will create
 * 2-ingredient proposals (gauze + alcohol) on alcohol-swab products already
 * matched by batch 7, but all proposals are status='proposed' for pharmacist
 * review so this is harmless — the reviewer separates them.
 *
 * preferredCategory = "วัสดุทำแผล" (wound care supplies).
 * Same safe/idempotent pattern as batches 1-9. Default --dry-run; --commit persists.
 *   node scripts/seed_ingredient_dictionary_batch10.js [--dry-run|--commit] [--db-url <url>]
 */
...
const SOURCE = "seed_dictionary_batch_10";
const WOUND = "ทำแผล";

const MODELING_NOTES = [
  '"gauze" is a product-type/shelf term, not a true active ingredient — seeded so the review queue can propose the wound care supplies shelf.',
  'First batch with Thai synonyms. Thai synonyms are stored for admin search but do NOT drive matching (matcher is Latin-only; normalizeLatin strips Thai to non-Latin tokens, which loadActiveSynonyms guards against).',
  'Bare "swab" is included per user decision — will create 2-ingredient (gauze + alcohol) proposals on alcohol-swab products from batch 7; separated at pharmacist review.',
  'preferredCategory = "ทำแผล" (wound dressing — confirmed category).',
];

const INGREDIENTS = [
  {
    canonical: "gauze",
    display: "Gauze / Wound Dressing",
    drugClass: "Medical supply (Wound care)",
    indications: ["Wound care/Dressing"],
    preferredCategory: WOUND,
    uncertainReason: `Wound dressing shelf "${WOUND}" not present in confirmed categories.`,
    synonyms: [
      // ── English ───────────────────────────────────────────────────────────
      "gauze",
      "gauze pad",
      ...
      "sterile compress",
      "gauze compress",
      // ── Thai (stored for admin search; not used by Latin-only matcher) ────
      "ก๊อซ",
      "ผ้าก๊อซ",
      "ผ้าก๊อส",
      "ก๊อซทำแผล",
    ...
    for (const def of INGREDIENTS) {
      ...
      if (resolvedCategory) {
        const ruleInserted = await insertCategoryRuleIfMissing(client, {
          ...
          note: `Batch 10 gauze rule: ${def.display} -> ${resolvedCategory}`,
        });
...
function printSummary(stats) {
  const lines = [];
  lines.push("==================================================");
  lines.push(` INGREDIENT DICTIONARY SEED — BATCH 10 (gauze/wound dressing)  [${stats.mode.toUpperCase()}]`);
  lines.push("==================================================");
...
async function main() {
  ...
  if (args.help) {
    console.log("node scripts/seed_ingredient_dictionary_batch10.js [--dry-run|--commit] [--db-url <url>]");
    return;
  }
...
if (require.main === module) {
  main().catch((error) => {
    console.error(`Batch 10 seed failed: ${error.message}`);
    process.exitCode = 1;
  });
}
