...
/**
 * Ingredient Dictionary Seed — Batch 8 (Set 8: Throat Lozenge Antiseptics).
 *
 * Models two antiseptic active ingredients commonly found in throat lozenges
 * (e.g. Strepsils):
 *   1. 2,4-Dichlorobenzyl Alcohol (DCBA)
 *   2. Amylmetacresol (AMC)
 *
 * They are kept as TWO separate ingredients (like glucosamine + chondroitin in
 * batch 5) so products containing only one of them are matched correctly.
 * The backfill handles multi-ingredient products (2+ candidates) cleanly.
 *
 * Both map to preferredCategory = "ยาอม" (throat lozenges).
 *
 * normalizeLatin collapses hyphens/commas/spaces to single spaces, so
 * "2,4-Dichlorobenzyl Alcohol" -> "2 4 dichlorobenzyl alcohol" — the synonyms
 * below use the normalized forms the matcher will actually see.
 *
 * English-only synonyms; matcher is Latin-only.
 * Same safe/idempotent pattern as batches 1-7. Default --dry-run; --commit persists.
 *   node scripts/seed_ingredient_dictionary_batch8.js [--dry-run|--commit] [--db-url <url>]
 */
...
const SOURCE = "seed_dictionary_batch_8";
const LOZENGE = "ยาอม";

const MODELING_NOTES = [
  "Two separate ingredients (DCBA + AMC) to correctly match products containing only one of the two Strepsils actives.",
  'normalizeLatin collapses "2,4-" prefix: "2,4-dichlorobenzyl alcohol" -> "2 4 dichlorobenzyl alcohol"; all synonym forms below reflect the normalized token stream.',
  'Short abbreviations "DCBA" and "AMC" are included — they are widely used on Thai pharmacy labeling and are sufficiently distinct (4+ chars, not generic).',
  'Both map to "ยาอม" (throat lozenge) shelf.',
  "English-only synonyms; matcher is Latin-only, so Thai synonyms would not drive matching yet.",
];

const INGREDIENTS = [
  {
    canonical: "2,4-dichlorobenzyl alcohol",
    display: "2,4-Dichlorobenzyl Alcohol",
    drugClass: "Topical antiseptic (oral)",
    indications: ["Throat infection", "Oral antisepsis"],
    preferredCategory: LOZENGE,
    uncertainReason: `Lozenge shelf "${LOZENGE}" not present in confirmed categories.`,
    synonyms: [
      // Full names — normalizeLatin turns "2,4-" into "2 4 " so include both
      "2,4-dichlorobenzyl alcohol",
      "2 4 dichlorobenzyl alcohol",
      "dichlorobenzyl alcohol",
      // Abbreviation
      "dcba",
      // Common combo label forms
      "dichlorobenzyl",
    ],
  },
  {
    canonical: "amylmetacresol",
    ...
    synonyms: [
      ...
      // Abbreviation
      "amc",
      // IUPAC / chemical name occasionally printed on packaging
    ...
    for (const def of INGREDIENTS) {
      ...
      if (resolvedCategory) {
        const ruleInserted = await insertCategoryRuleIfMissing(client, {
          ...
          note: `Batch 8 lozenge antiseptic rule: ${def.display} -> ${resolvedCategory}`,
        });
...
function printSummary(stats) {
  const lines = [];
  lines.push("==================================================");
  lines.push(` INGREDIENT DICTIONARY SEED — BATCH 8 (throat lozenge antiseptics)  [${stats.mode.toUpperCase()}]`);
  lines.push("==================================================");
...
async function main() {
  ...
  if (args.help) {
    console.log("node scripts/seed_ingredient_dictionary_batch8.js [--dry-run|--commit] [--db-url <url>]");
    return;
  }
...
if (require.main === module) {
  main().catch((error) => {
    console.error(`Batch 8 seed failed: ${error.message}`);
    process.exitCode = 1;
  });
}
