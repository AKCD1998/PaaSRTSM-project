...
/**
 * Ingredient Dictionary Seed — Batch 12 (Heart/Cardiovascular + Brain/Memory families).
 *
 * Part of Knowledge Layer v1 — Ingredient Family Dictionary for Thai pharmacy.
 *
 * HEART / CARDIOVASCULAR (1 new family — OMEGA3/KRILL_OIL already seeded in batch 5):
 *   COQ10
 *
 * BRAIN / MEMORY (4 families):
 *   GINKGO, PHOSPHATIDYLSERINE, BACOPA, LIONS_MANE
 *
 * Already seeded — skipped:
 *   OMEGA3 / FISH_OIL (batch 5), KRILL_OIL (batch 5, under fish oil synonyms)
 *
 * Abbreviation decisions:
 *   "ps"  — 2-char for phosphatidylserine — EXCLUDED (massive false-positive risk)
 *   "egb761" + "egb 761" — both included: normalizeLatin turns "EGb 761" -> "egb 761"
 *     and "EGb761" -> "egb761" so two forms needed to catch both label styles
 *   "co q10" — included: common label spacing variant of coq10
 *
 * normalizeLatin note: "lion's mane" -> "lion s mane" (apostrophe -> space),
 *   so "lion's mane" and "lions mane" produce DIFFERENT match strings and both
 *   are kept to cover both label styles.
 *
 * All 5 families → preferredCategory = "วิตามิน".
 *
 * Same safe/idempotent pattern as batches 1-11. Default --dry-run; --commit persists.
 *   node scripts/seed_ingredient_dictionary_batch12.js [--dry-run|--commit] [--db-url <url>]
 */
...
const SOURCE = "seed_dictionary_batch_12";
const VITAMIN = "วิตามิน";

const MODELING_NOTES = [
  "Batch 12 = KL-v1 Heart/Cardiovascular + Brain/Memory: 5 families (COQ10, GINKGO, PHOSPHATIDYLSERINE, BACOPA, LIONS_MANE).",
  "OMEGA3 and KRILL_OIL already seeded in batch 5 — skipped.",
  '"ps" excluded: 2-char abbreviation for phosphatidylserine — false-positive risk too high.',
  '"egb761" and "egb 761" both included: normalizeLatin maps "EGb761"->"egb761" and "EGb 761"->"egb 761" — different match strings.',
  '"lion\'s mane" and "lions mane" both kept: apostrophe normalizes to space so they produce different match strings (" lion s mane " vs " lions mane ").',
...
const INGREDIENTS = [
  ...
  {
    canonical: "coq10",
    display: "CoQ10 (Coenzyme Q10)",
    drugClass: "Antioxidant (Coenzyme Q10)",
    indications: ["Cardiovascular", "Antioxidant", "Energy supplement"],
  ...
  // ── BRAIN / MEMORY ────────────────────────────────────────────────────────
  {
    ...
    drugClass: "Herbal extract (Brain/Memory)",
    indications: ["Brain/Memory", "Circulation"],
    preferredCategory: VITAMIN,
    synonyms: [
      "ginkgo",
      "ginkgo biloba",
      ...
      "egb761",
      "egb 761",
      // egb761 = standardized extract (EGb 761, Schwabe) — two normalizeLatin forms
    ],
  },
  {
    canonical: "phosphatidylserine",
    display: "Phosphatidylserine",
    drugClass: "Phospholipid (Brain/Memory)",
    indications: ["Brain/Memory"],
    preferredCategory: VITAMIN,
    synonyms: [
      "phosphatidylserine",
      "sharp-ps",
      "sharp ps",
      // "ps" excluded: 2-char — too much false-positive risk
    ],
  },
  {
    canonical: "bacopa",
    display: "Bacopa Monnieri",
    drugClass: "Herbal extract (Brain/Memory)",
    indications: ["Brain/Memory"],
    preferredCategory: VITAMIN,
    synonyms: [
  ...
  },
  {
    ...
    display: "Lion's Mane Mushroom",
    drugClass: "Medicinal mushroom (Brain/Memory)",
    indications: ["Brain/Memory", "Immune support"],
    preferredCategory: VITAMIN,
    synonyms: [
      "lion's mane",
      "lions mane",
      "lion s mane",
      // all three forms: apostrophe and space variants after normalizeLatin
      "hericium erinaceus",
      "hericium",
    ...
    for (const def of INGREDIENTS) {
      ...
      if (resolvedCategory) {
        const ruleInserted = await insertCategoryRuleIfMissing(client, {
          ...
          note: `Batch 12 KL-v1 rule: ${def.display} -> ${resolvedCategory}`,
        });
...
function printSummary(stats) {
  const lines = [];
  lines.push("==================================================");
  lines.push(` INGREDIENT DICTIONARY SEED — BATCH 12 (Heart + Brain)  [${stats.mode.toUpperCase()}]`);
  lines.push("==================================================");
...
async function main() {
  ...
  if (args.help) {
    console.log("node scripts/seed_ingredient_dictionary_batch12.js [--dry-run|--commit] [--db-url <url>]");
    return;
  }
...
if (require.main === module) {
  main().catch((error) => {
    console.error(`Batch 12 seed failed: ${error.message}`);
    process.exitCode = 1;
  });
}
