...
/**
 * Ingredient Dictionary Seed — Batch 30 (กระชาย / Fingerroot / Boesenbergia rotunda).
 *
 * Single ingredient (one accepted species with one older synonym name).
 *
 * BOTANICAL SYNONYM NOTE:
 *   Boesenbergia rotunda  = current accepted name (POWO / Plants of the World Online)
 *   Boesenbergia pandurata = older synonym; still appears on older product labels and
 *                            research papers (panduratin is named after this synonym)
 *   Both botanical names are included as synonyms of the SAME ingredient entry.
 *
 * KEY CORRECTION vs BATCH 27 NOTE:
 *   Batch 27 modeling notes stated "กระชาย = Kaempferia galanga (aromatic ginger)" —
 *   this was inaccurate. กระชาย in Thai pharmacy universally refers to Boesenbergia
 *   rotunda (fingerroot). Kaempferia galanga is known as เปราะหอม / กาหลง in Thai.
 *   The batch 27 decision to exclude "กระชาย" from กระชายดำ was still correct (wrong
 *   reason, right outcome). "กระชาย" is assigned here to B. rotunda.
 *   Disambiguation still applies: กระชาย ≠ กระชายดำ — different species, different use.
 *
 * PANDURATIN note:
 *   Panduratin (panduratin A) is named after the older species name B. pandurata.
 *   It is the primary bioactive compound and appears as a standalone potency claim on
 *   some supplement labels ("panduratin Xmg") — included by the same rule as
 *   gingerol (batch 24), charantin (batch 23), asiaticoside (batch 26).
 *   Unlike the batch 27 note, this compound genuinely appears alone on labels.
 *   กระชายดำ fame peaked during COVID-19 research 2020–2022 (antiviral activity).
 *
 * Trimmed from user's list:
 *   "Boesenbergia rotunda (L.) Mansf."       — "(L.) Mansf." = author notation
 *   "Boesenbergia pandurata (Roxb.) Schltr." — "(Roxb.) Schltr." = author notation
 *
 * Added vs user's list:
 *   "สมุนไพรกระชาย" — herbal-prefix form on Thai supplement labels
 *   "กระชายสด"      — fresh rhizome; appears on traditional/fresh herb product labels
 *   "krachai"        — romanized Thai name on export/bilingual labels
 *   "panduratin"     — primary bioactive; standalone potency claim on supplement labels
 *   "chinese keys"   — English name from rhizome shape; appears on export/specialty labels
 *
 * Thai synonyms (8): กระชาย, กระชายขาว, กระชายเหลือง, เหง้ากระชาย,
 *   สารสกัดกระชาย, กระชายขาวสกัด, สมุนไพรกระชาย, กระชายสด
 * English/scientific (10): boesenbergia rotunda, boesenbergia pandurata,
 *   boesenbergia rotunda extract, fingerroot, finger root, fingerroot extract,
 *   thai fingerroot, krachai, panduratin, chinese keys
 *
 * preferredCategory = "ยาสมุนไพร" — skipped gracefully if not confirmed in DB.
 *
 * Default --dry-run; --commit persists.
 *   node scripts/seed_ingredient_dictionary_batch30.js [--dry-run|--commit] [--db-url <url>]
 */
...
const SOURCE = "seed_dictionary_batch_30";
const PREFERRED_CATEGORY = "ยาสมุนไพร";

const MODELING_NOTES = [
  "Batch 30 = กระชาย / Fingerroot (Boesenbergia rotunda) — antiviral, anti-inflammatory, digestive tonic.",
  "BOTANICAL SYNONYM: Boesenbergia pandurata = older accepted name for same species as B. rotunda; both included.",
  "CORRECTION vs batch 27: กระชาย refers to Boesenbergia rotunda, NOT Kaempferia galanga.",
  "Kaempferia galanga = เปราะหอม / กาหลง in Thai — a different plant entirely.",
  "Disambiguation still holds: กระชาย (B. rotunda) ≠ กระชายดำ (K. parviflora / batch 27).",
  "'Boesenbergia rotunda (L.) Mansf.' trimmed: '(L.) Mansf.' = botanical author notation.",
  "'Boesenbergia pandurata (Roxb.) Schltr.' trimmed: '(Roxb.) Schltr.' = botanical author notation.",
  "'panduratin' added: primary bioactive; appears as standalone potency claim on supplement labels.",
  "  Named after old species name B. pandurata. Panduratin A / antiviral research peaked 2020-2022.",
  "'กระชายสด' added: fresh rhizome form on traditional / fresh herb product labels.",
  "'krachai' added: romanized Thai name on export/bilingual labels.",
  "'chinese keys' added: English name from rhizome shape on export/specialty labels.",
  "กระชายขาว and กระชายเหลือง are color descriptors for the same B. rotunda rhizome, not distinct species.",
  "preferredCategory 'ยาสมุนไพร' — skipped gracefully if not yet confirmed in product_category_states.",
];

const INGREDIENTS = [
  {
    canonical: "boesenbergia rotunda",
    display: "Fingerroot / Krachai (กระชาย / Boesenbergia rotunda)",
    drugClass: "Herbal antiviral / Anti-inflammatory (Boesenbergia / Fingerroot)",
    indications: [
      "Antiviral (herbal)",
      "Anti-inflammatory (herbal)",
      "Digestive tonic (herbal)",
    ],
    ...
    synonyms: [
      ...
      "thai fingerroot",
      "chinese keys",
      // ── Romanized / marketing names ───────────────────────────────────────
      "krachai",
      // ── Primary bioactive (standalone label claim) ────────────────────────
      "panduratin",
    ],
  },
];
    ...
    for (const def of INGREDIENTS) {
      ...
      if (resolvedCategory) {
        const ruleInserted = await insertCategoryRuleIfMissing(client, {
          ...
          note: `Batch 30 rule: ${def.display} -> ${resolvedCategory}`,
        });
...
function printSummary(stats) {
  const lines = [];
  lines.push("==================================================");
  lines.push(` INGREDIENT DICTIONARY SEED — BATCH 30 (กระชาย / Fingerroot)  [${stats.mode.toUpperCase()}]`);
  lines.push("==================================================");
...
async function main() {
  ...
  if (args.help) { console.log("node scripts/seed_ingredient_dictionary_batch30.js [--dry-run|--commit] [--db-url <url>]"); return; }
...
if (require.main === module) {
  main().catch((err) => { console.error(`Batch 30 seed failed: ${err.message}`); process.exitCode = 1; });
}
