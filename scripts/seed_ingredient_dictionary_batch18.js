...
/**
 * Ingredient Dictionary Seed — Batch 18 (มะขามแขก / Senna).
 *
 * Single ingredient family. Sennosides are the active anthraquinone glycosides
 * found in both leaf and pod; they are always from the same plant and seeded
 * as synonyms rather than separate ingredients.
 *
 * Trimmed from user's list:
 *   "sennoside A", "sennoside B" — sub-component level, rarely appear as
 *   standalone tokens on Thai pharmacy product labels; "sennosides" covers them.
 *
 * Added vs user's list:
 *   "senna pod" (singular), "senna leaf extract", "senna pod extract",
 *   "senna glycosides" (synonym for sennosides), "cassia senna" (old name),
 *   "สมุนไพรมะขามแขก", "แคสเซีย" (Thai transliteration used on some labels)
 *
 * Thai synonyms (6): มะขามแขก, ใบมะขามแขก, ฝักมะขามแขก, สารสกัดมะขามแขก,
 *                    สมุนไพรมะขามแขก, แคสเซีย
 * English/scientific (16): senna, senna leaf, senna pod, senna pods,
 *   senna extract, senna leaf extract, senna pod extract, sennoside, sennosides,
 *   senna glycosides, Senna alexandrina, Cassia angustifolia, Cassia acutifolia,
 *   cassia senna, Sennae folium, Sennae fructus
 *
 * preferredCategory = "ยาระบาย" — skipped gracefully if not confirmed in DB.
 *
 * Default --dry-run; --commit persists.
 *   node scripts/seed_ingredient_dictionary_batch18.js [--dry-run|--commit] [--db-url <url>]
 */
...
const SOURCE = "seed_dictionary_batch_18";
const LAXATIVE_CATEGORY = "ยาระบาย";

const MODELING_NOTES = [
  "Batch 18 = มะขามแขก (Senna) — single family, leaf + pod + extract forms unified.",
  "'sennoside A' and 'sennoside B' excluded: sub-component level, not standalone label tokens; 'sennosides' covers them.",
  "'senna glycosides' added: common pharmacopoeia synonym for sennosides used in some Thai product documents.",
  "Both current botanical name (Senna alexandrina) and legacy Cassia names included: legacy names still appear on many existing product labels.",
  "Latin pharmacopoeia terms (Sennae folium, Sennae fructus) included: appear on imported products and reference imports.",
  "'แคสเซีย' added: Thai transliteration used on some traditional medicine labels.",
  "preferredCategory 'ยาระบาย' — skipped gracefully if not yet confirmed in product_category_states.",
];

const INGREDIENTS = [
  {
    canonical: "senna",
    display: "Senna (มะขามแขก)",
    drugClass: "Stimulant laxative (Anthraquinone)",
    indications: ["Constipation", "Bowel preparation"],
    preferredCategory: LAXATIVE_CATEGORY,
    synonyms: [
      // ── Thai names ─────────────────────────────────────────────────────────
      "มะขามแขก",
      "ใบมะขามแขก",
    ...
    for (const def of INGREDIENTS) {
      ...
      if (resolvedCategory) {
        const ruleInserted = await insertCategoryRuleIfMissing(client, {
          ...
          note: `Batch 18 rule: ${def.display} -> ${resolvedCategory}`,
        });
...
function printSummary(stats) {
  const lines = [];
  lines.push("==================================================");
  lines.push(` INGREDIENT DICTIONARY SEED — BATCH 18 (มะขามแขก / Senna)  [${stats.mode.toUpperCase()}]`);
  lines.push("==================================================");
  ...
  if (stats.uncertainCategoryMappings.length) {
    ...
    lines.push("  → Add 'ยาระบาย' via Review Queue first, then re-run --commit to attach the rule.");
  }
...
async function main() {
  ...
  if (args.help) {
    console.log("node scripts/seed_ingredient_dictionary_batch18.js [--dry-run|--commit] [--db-url <url>]");
    return;
  }
...
if (require.main === module) {
  main().catch((error) => {
    console.error(`Batch 18 seed failed: ${error.message}`);
    process.exitCode = 1;
  });
}
