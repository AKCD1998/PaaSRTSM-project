...
/**
 * Ingredient Dictionary Seed — Batch 22 (ยูเรีย / Urea).
 *
 * Single ingredient. Topical urea is a keratolytic and humectant used in
 * moisturizing creams and lotions at varying concentrations:
 *   2–10%  = humectant / moisturizer
 *   20–40% = keratolytic (dead-skin removal, callus, hyperkeratosis)
 *   40%+   = nail softener
 * Concentration is a product-level detail — the ingredient is simply "urea".
 *
 * IMPORTANT — do NOT merge with related compounds:
 *   "carbamide peroxide" = urea + hydrogen peroxide (tooth whitening, ear drops)
 *   "hydroxyurea / hydroxycarbamide" = anticancer drug — completely different
 *   "urea peroxide" = another name for carbamide peroxide
 * "carbamide" alone IS a valid synonym for plain urea; "carbamide peroxide" is NOT.
 *
 * Trimmed from user's list:
 *   "INCI Urea"     — "INCI" is a standard-qualifier prefix, not a label token.
 *                     The INCI name is simply "Urea", already in the list.
 *   "CAS 57-13-6"   — CAS registry format; never appears on pharmacy product labels.
 *
 * Added vs user's list:
 *   "urea ointment" — ointment base form; found in some compounded/OTC products.
 *
 * Thai synonyms (4): ยูเรีย, ยูเรียครีม, ครีมยูเรีย, ยูเรียโลชั่น
 * English/scientific (6): urea, urea cream, urea lotion, urea ointment,
 *                          topical urea, carbamide
 *
 * preferredCategory = "เวชสำอางค์" — skipped gracefully if not confirmed in DB.
 *
 * Default --dry-run; --commit persists.
 *   node scripts/seed_ingredient_dictionary_batch22.js [--dry-run|--commit] [--db-url <url>]
 */
...
const SOURCE = "seed_dictionary_batch_22";
const PREFERRED_CATEGORY = "เวชสำอางค์";

const MODELING_NOTES = [
  "Batch 22 = ยูเรีย / Urea — topical keratolytic and humectant; concentration is a product-level detail.",
  "Do NOT merge 'carbamide peroxide' here: urea peroxide = tooth whitening/ear drops — separate ingredient.",
  "Do NOT merge 'hydroxyurea / hydroxycarbamide' here: anticancer drug, completely unrelated.",
  "'carbamide' alone IS a valid urea synonym; 'carbamide peroxide' is NOT.",
  "'INCI Urea' excluded: 'INCI' is a standard-qualifier prefix; the INCI name is just 'Urea', already in list.",
  "'CAS 57-13-6' excluded: CAS registry format; never appears on Thai pharmacy product labels.",
  "'urea ointment' added: ointment form found in some OTC and compounded products.",
  "Drug class: 'Keratolytic / Emollient (Urea)' — covers both moisturizing and exfoliating indications.",
  "preferredCategory 'เวชสำอางค์' — skipped gracefully if not yet confirmed in product_category_states.",
];

const INGREDIENTS = [
  {
    canonical: "urea topical",
    display: "Urea (ยูเรีย) — Topical",
    drugClass: "Keratolytic / Emollient (Urea)",
    indications: ["Dry skin / Xerosis (topical)", "Hyperkeratosis / Callus (keratolytic)", "Nail softening (topical)"],
    ...
    for (const def of INGREDIENTS) {
      ...
      if (resolvedCategory) {
        const ruleInserted = await insertCategoryRuleIfMissing(client, {
          ...
          note: `Batch 22 rule: ${def.display} -> ${resolvedCategory}`,
        });
...
function printSummary(stats) {
  const lines = [];
  lines.push("==================================================");
  lines.push(` INGREDIENT DICTIONARY SEED — BATCH 22 (ยูเรีย / Urea)  [${stats.mode.toUpperCase()}]`);
  lines.push("==================================================");
  ...
  if (stats.uncertainCategoryMappings.length) {
    ...
    lines.push("  → Confirm 'เวชสำอางค์' via Review Queue, then re-run --commit to attach the rule.");
  }
...
async function main() {
  ...
  if (args.help) {
    console.log("node scripts/seed_ingredient_dictionary_batch22.js [--dry-run|--commit] [--db-url <url>]");
    return;
  }
...
if (require.main === module) {
  main().catch((error) => {
    console.error(`Batch 22 seed failed: ${error.message}`);
    process.exitCode = 1;
  });
}
