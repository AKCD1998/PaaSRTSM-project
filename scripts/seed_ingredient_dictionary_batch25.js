...
/**
 * Ingredient Dictionary Seed — Batch 25 (โพรพอลิส / Propolis).
 *
 * Single ingredient. Propolis is a resinous mixture collected by honeybees
 * from plant sources, used as an antimicrobial, antioxidant, and
 * immunomodulating supplement. It is a bee product (not a plant extract),
 * so product_type would be 'supplement' not 'herb' in the future taxonomy.
 *
 * Thai spelling note: both "โพรพอลิส" and "โพรโพลิส" are in active use on
 * Thai product labels — both are included. Neither is a misspelling.
 *
 * Trimmed from user's list:
 *   "Propolis Extract" (title-case) — duplicate of "propolis extract";
 *   the synonym table uses LOWER(BTRIM()) for deduplication.
 *
 * Added vs user's list:
 *   "สารสกัดกาวผึ้ง" — Thai extract form using the literal "กาวผึ้ง" name
 *   "propolis liquid"  — liquid-concentrate form found on some product labels
 *
 * Thai synonyms (6): โพรพอลิส, โพรโพลิส, สารสกัดโพรพอลิส,
 *                    สารสกัดโพรโพลิส, กาวผึ้ง, สารสกัดกาวผึ้ง
 * English (7): propolis, propolis extract, bee propolis, bee glue,
 *              propolis resin, propolis tincture, propolis liquid
 *
 * preferredCategory = "อาหารเสริม" — skipped gracefully if not confirmed in DB.
 *
 * Default --dry-run; --commit persists.
 *   node scripts/seed_ingredient_dictionary_batch25.js [--dry-run|--commit] [--db-url <url>]
 */
...
const SOURCE = "seed_dictionary_batch_25";
const PREFERRED_CATEGORY = "อาหารเสริม";

const MODELING_NOTES = [
  "Batch 25 = โพรพอลิส / Propolis — bee resin product; product_type = supplement (not herb).",
  "Both Thai spellings included: 'โพรพอลิส' and 'โพรโพลิส' — both appear on Thai product labels.",
  "'Propolis Extract' (title-case) excluded: duplicate of lowercase form; LOWER() dedup handles it.",
  "'สารสกัดกาวผึ้ง' added: Thai extract form using the 'กาวผึ้ง' (bee glue) literal name.",
  "'propolis liquid' added: liquid-concentrate label form.",
  "Propolis tincture contains alcohol solvent — note for allergy/sensitivity audit if needed.",
...
const INGREDIENTS = [
  {
    canonical: "propolis",
    display: "Propolis (โพรพอลิส / กาวผึ้ง)",
    drugClass: "Herbal antimicrobial / Immunomodulator (Propolis)",
    indications: ["Immunomodulation", "Antimicrobial (herbal)", "Antioxidant"],
    ...
    for (const def of INGREDIENTS) {
      ...
      if (resolvedCategory) {
        const ruleInserted = await insertCategoryRuleIfMissing(client, {
          ...
          note: `Batch 25 rule: ${def.display} -> ${resolvedCategory}`,
        });
...
function printSummary(stats) {
  const lines = [];
  lines.push("==================================================");
  lines.push(` INGREDIENT DICTIONARY SEED — BATCH 25 (โพรพอลิส / Propolis)  [${stats.mode.toUpperCase()}]`);
  lines.push("==================================================");
...
async function main() {
  ...
  if (args.help) {
    console.log("node scripts/seed_ingredient_dictionary_batch25.js [--dry-run|--commit] [--db-url <url>]");
    return;
  }
...
if (require.main === module) {
  main().catch((error) => {
    console.error(`Batch 25 seed failed: ${error.message}`);
    process.exitCode = 1;
  });
}
