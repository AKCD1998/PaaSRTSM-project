...
/**
 * Ingredient Dictionary Seed — Batch 28 (กระเจี๊ยบแดง / Roselle / Hibiscus sabdariffa).
 *
 * Single ingredient family. The medicinal part used is the dried calyx
 * (กลีบเลี้ยง), not the flower petals; Thai labels commonly use "ดอก"
 * (flower) as a loose term for the calyx — both forms are included.
 *
 * PRIMARY BIOACTIVES: anthocyanins, hibiscus acid (hydroxycitric acid),
 * polyphenols. These compounds are not individually listed here as standalone
 * synonyms because "anthocyanins" is too generic (present in many plants) and
 * "hydroxycitric acid" may cause confusion with garcinia cambogia products.
 *
 * SPECIES DISAMBIGUATION (encoded in MODELING_NOTES):
 *   กระเจี๊ยบแดง = Hibiscus sabdariffa (roselle) ← THIS ingredient
 *   กระเจี๊ยบเขียว = Abelmoschus esculentus (okra) ← different plant
 *   "กระเจี๊ยบ" alone is NOT included — ambiguous with okra.
 *   In cosmetics, INCI "hibiscus extract" may refer to Hibiscus rosa-sinensis
 *   (ชบา), not H. sabdariffa — verify during audit if a cosmetic product matches.
 *
 * Trimmed from user's list:
 *   "Hibiscus sabdariffa L." — "L." is a botanical author abbreviation; not a
 *                              product label token (consistent rule, all batches).
 *
 * Added vs user's list:
 *   "สมุนไพรกระเจี๊ยบแดง" — herbal-prefix form used on Thai supplement labels
 *   "roselle tea"          — English tea-product form (pairs with ชากระเจี๊ยบแดง)
 *   "hibiscus"             — shortened name on tea/supplement labels; in Thai
 *                            pharmacy context almost always means H. sabdariffa
 *   "hibiscus tea"         — very common product form label
 *
 * Thai synonyms (8): กระเจี๊ยบแดง, ดอกกระเจี๊ยบแดง, กลีบเลี้ยงกระเจี๊ยบแดง,
 *   สารสกัดกระเจี๊ยบแดง, กระเจี๊ยบแดงสกัด, น้ำกระเจี๊ยบแดง,
 *   ชากระเจี๊ยบแดง, สมุนไพรกระเจี๊ยบแดง
 * English/scientific (10): hibiscus sabdariffa, hibiscus sabdariffa extract,
 *   roselle, roselle extract, roselle calyx extract, roselle tea,
 *   jamaica sorrel, red sorrel, hibiscus, hibiscus tea
 *
 * preferredCategory = "ยาสมุนไพร" — skipped gracefully if not confirmed in DB.
 *
 * Default --dry-run; --commit persists.
 *   node scripts/seed_ingredient_dictionary_batch28.js [--dry-run|--commit] [--db-url <url>]
 */
...
const SOURCE = "seed_dictionary_batch_28";
const PREFERRED_CATEGORY = "ยาสมุนไพร";

const MODELING_NOTES = [
  "Batch 28 = กระเจี๊ยบแดง / Roselle (Hibiscus sabdariffa) — calyx extract; antioxidant, blood pressure support.",
  "Used part is the dried calyx (กลีบเลี้ยง); 'ดอก' on Thai labels refers loosely to the calyx, not flower petals.",
  "DISAMBIGUATION: กระเจี๊ยบแดง (Hibiscus sabdariffa) ≠ กระเจี๊ยบเขียว (Abelmoschus esculentus / okra).",
  "'กระเจี๊ยบ' alone excluded: ambiguous with okra (Abelmoschus esculentus).",
  "COSMETIC CAUTION: INCI 'hibiscus extract' may refer to Hibiscus rosa-sinensis (ชบา), not H. sabdariffa — verify cosmetic product matches during audit.",
  "'Hibiscus sabdariffa L.' excluded: 'L.' is botanical author abbreviation, not a product label token.",
  "'สมุนไพรกระเจี๊ยบแดง' added: herbal-prefix form on Thai supplement labels.",
  "'roselle tea' and 'hibiscus tea' added: tea-product label forms.",
  "'hibiscus' added: shortened name on tea/supplement labels; in Thai pharmacy context = H. sabdariffa.",
  "Anthocyanins and hibiscus acid NOT added as synonyms: too generic / confusion risk with other plants.",
  "preferredCategory 'ยาสมุนไพร' — skipped gracefully if not yet confirmed in product_category_states.",
];

const INGREDIENTS = [
  {
    canonical: "hibiscus sabdariffa",
    display: "Roselle (กระเจี๊ยบแดง / Hibiscus sabdariffa)",
    drugClass: "Herbal antioxidant / Antihypertensive (Hibiscus / Anthocyanin)",
    indications: [
      "Antioxidant",
      "Blood pressure support (herbal)",
      "Antihyperlipidemic (herbal)",
    ],
    ...
    for (const def of INGREDIENTS) {
      ...
      if (resolvedCategory) {
        const ruleInserted = await insertCategoryRuleIfMissing(client, {
          ...
          note: `Batch 28 rule: ${def.display} -> ${resolvedCategory}`,
        });
...
function printSummary(stats) {
  const lines = [];
  lines.push("==================================================");
  lines.push(` INGREDIENT DICTIONARY SEED — BATCH 28 (กระเจี๊ยบแดง / Roselle)  [${stats.mode.toUpperCase()}]`);
  lines.push("==================================================");
...
async function main() {
  ...
  if (args.help) { console.log("node scripts/seed_ingredient_dictionary_batch28.js [--dry-run|--commit] [--db-url <url>]"); return; }
...
if (require.main === module) {
  main().catch((err) => { console.error(`Batch 28 seed failed: ${err.message}`); process.exitCode = 1; });
}
