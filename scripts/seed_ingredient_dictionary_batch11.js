...
/**
 * Ingredient Dictionary Seed — Batch 11 (Beauty/Skin + Eye Health families).
 *
 * Part of Knowledge Layer v1 — Ingredient Family Dictionary for Thai pharmacy.
 * Each entry is a canonical FAMILY (not a single salt/form) with subtypes as
 * synonyms. Idempotent: already-seeded synonyms skip gracefully.
 *
 * BEAUTY / SKIN (5 families):
 *   ASTAXANTHIN, GRAPE_SEED, CERAMIDE, HYALURONIC_ACID, ELASTIN
 *
 * EYE HEALTH (4 families):
 *   LUTEIN, ZEAXANTHIN, BILBERRY, MARIGOLD
 *
 * Abbreviation exclusions (noted per family):
 *   "ha"  — 2-char, would match any product containing the word "ha"
 *   "zea" — 3-char, false-positive risk
 *   "gse" — 3-char, false-positive risk
 *   "opc"/"opcs" — 3-4 char but kept: highly distinctive OPC supplement labeling
 *   "asta" — 4-char, kept: appears as standalone brand token (e.g. "ASTA 4MG")
 *
 * anthocyanin under BILBERRY: this is the primary active found in bilberry but
 * will also match other anthocyanin-rich products (elderberry, blueberry) —
 * acceptable at this stage since all rows are proposed for pharmacist review.
 *
 * All 9 families → preferredCategory = "วิตามิน" (oral supplement shelf).
 *
 * Same safe/idempotent pattern as batches 1-10. Default --dry-run; --commit persists.
 *   node scripts/seed_ingredient_dictionary_batch11.js [--dry-run|--commit] [--db-url <url>]
 */
...
const SOURCE = "seed_dictionary_batch_11";
const VITAMIN = "วิตามิน";

const MODELING_NOTES = [
  "Batch 11 = Knowledge Layer v1 Ingredient Family Dictionary: 9 families (Beauty/Skin + Eye Health).",
  "Each canonical is a FAMILY; subtypes/salts/brands are synonyms.",
  "Abbreviation exclusions: 'ha' (2-char), 'zea' (3-char), 'gse' (3-char) — false-positive risk too high.",
  "'opc'/'opcs' kept: highly distinctive OPC supplement labeling; 'asta' kept: appears as standalone brand token.",
  "'anthocyanin' under BILBERRY also matches elderberry/blueberry — acceptable, caught at pharmacist review.",
  "All families → วิตามิน shelf (oral supplement products).",
];
...
const INGREDIENTS = [
  // ── BEAUTY / SKIN ─────────────────────────────────────────────────────────
  {
    ...
    display: "Astaxanthin",
    drugClass: "Antioxidant (Carotenoid)",
    indications: ["Antioxidant", "Skin/Beauty supplement"],
    ...
    synonyms: [
      ...
      "bioastin",
      "asta",
      // "asta" kept: distinctive supplement brand token (e.g. "ASTA REAL 4MG")
    ],
  },
  {
    canonical: "grape seed",
    display: "Grape Seed Extract",
    drugClass: "Antioxidant (Polyphenol/OPC)",
    indications: ["Antioxidant", "Skin/Beauty supplement", "Cardiovascular"],
    preferredCategory: VITAMIN,
    synonyms: [
      "grape seed",
      "grape seed extract",
      ...
      "proanthocyanidin",
      "proanthocyanidins",
      // "gse" excluded: 3-char, false-positive risk
    ],
  },
  {
    canonical: "ceramide",
    display: "Ceramide",
    drugClass: "Skin supplement (Ceramide)",
    indications: ["Skin/Beauty supplement"],
    preferredCategory: VITAMIN,
    synonyms: [
      "ceramide",
      "rice ceramide",
  ...
  },
  {
    ...
    drugClass: "Skin/Joint supplement (Hyaluronic acid)",
    indications: ["Skin/Beauty supplement", "Joint supplement"],
    preferredCategory: VITAMIN,
    synonyms: [
      "hyaluronic acid",
      "sodium hyaluronate",
      "hyaluronan",
      // "ha" excluded: 2-char, would match any product token "ha" — massive false positive
    ],
  },
  {
    canonical: "elastin",
    display: "Elastin",
    drugClass: "Skin supplement (Elastin)",
    indications: ["Skin/Beauty supplement"],
    preferredCategory: VITAMIN,
    synonyms: [
      "elastin",
  ...
  // ── EYE HEALTH ────────────────────────────────────────────────────────────
  {
    canonical: "lutein",
    display: "Lutein",
    drugClass: "Carotenoid (Eye health)",
  ...
  },
  {
    ...
    synonyms: [
      "zeaxanthin",
      // "zea" excluded: 3-char, false-positive risk
    ],
  },
  {
    canonical: "bilberry",
    display: "Bilberry",
    drugClass: "Herbal extract (Eye health/Antioxidant)",
    indications: ["Eye health", "Antioxidant"],
    preferredCategory: VITAMIN,
    synonyms: [
      "bilberry",
      "bilberry extract",
      "vaccinium myrtillus",
      "anthocyanin",
      // anthocyanin also matches elderberry/blueberry products — reviewed at proposal stage
    ],
  },
  {
    canonical: "marigold extract",
    display: "Marigold Extract",
    drugClass: "Herbal extract (Eye health)",
    indications: ["Eye health"],
    preferredCategory: VITAMIN,
    synonyms: [
      "marigold extract",
    ...
    for (const def of INGREDIENTS) {
      ...
      if (resolvedCategory) {
        const ruleInserted = await insertCategoryRuleIfMissing(client, {
          ...
          note: `Batch 11 KL-v1 rule: ${def.display} -> ${resolvedCategory}`,
        });
...
function printSummary(stats) {
  const lines = [];
  lines.push("==================================================");
  lines.push(` INGREDIENT DICTIONARY SEED — BATCH 11 (Beauty/Skin + Eye Health)  [${stats.mode.toUpperCase()}]`);
  lines.push("==================================================");
...
async function main() {
  ...
  if (args.help) {
    console.log("node scripts/seed_ingredient_dictionary_batch11.js [--dry-run|--commit] [--db-url <url>]");
    return;
  }
...
if (require.main === module) {
  main().catch((error) => {
    console.error(`Batch 11 seed failed: ${error.message}`);
    process.exitCode = 1;
  });
}
