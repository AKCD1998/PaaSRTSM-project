...
/**
 * Ingredient Dictionary Seed — Batch 6 (Set 6: Soap / Cleanser).
 *
 * Models a single broad "soap" category-ingredient covering bar soaps, syndet
 * bars, liquid cleansers and face/body washes. Unlike a true active ingredient,
 * "soap" is a product-type/shelf term — it is seeded so the review queue can
 * propose a soap shelf for personal-care cleansing products.
 *
 ...
 * The "soap" dose-form entry in the discovery STOPWORDS list is ONLY used by the
 * ranking report, NOT by the matcher, so the "soap" synonym matches products.
 * normalizeLatin collapses hyphens to spaces, so "soap-free cleanser" and
 * "soap free cleanser" normalize identically — both are kept (the second simply
 * inserts a redundant row; harmless and idempotent).
 *
 * Synonyms are exactly the curated SET 6 list (English). Thai forms are not
 * provided yet; the matcher is Latin-only so Thai would not drive matching today.
 *
 * Same safe/idempotent pattern as batches 1-5. Default --dry-run; --commit persists.
 *   node scripts/seed_ingredient_dictionary_batch6.js [--dry-run|--commit] [--db-url <url>]
 */
...
const SOURCE = "seed_dictionary_batch_6";
const SOAP = "สบู่";

const MODELING_NOTES = [
  '"soap" is a product-type/shelf term, not a true active ingredient — seeded so the review queue can propose a soap shelf for cleansing products.',
  'The discovery STOPWORDS list (which contains "soap") only affects the ranking report; the backfill matcher matches it normally.',
  'normalizeLatin collapses hyphens to spaces, so "soap-free cleanser" == "soap free cleanser" after normalization; both raw rows are kept (idempotent).',
  "English-only synonyms for now; matcher is Latin-only, so Thai synonyms would not drive matching yet.",
];

const INGREDIENTS = [
  {
    canonical: "soap",
    display: "Soap / Cleanser",
    drugClass: "Personal care (Soap/Cleanser)",
    indications: ["Skin cleansing"],
    preferredCategory: SOAP,
    uncertainReason: 'Soap shelf "สบู่" not present in confirmed categories.',
    synonyms: [
      "soap",
      "beauty soap",
      "toilet soap",
    ...
    for (const def of INGREDIENTS) {
      ...
      if (resolvedCategory) {
        const ruleInserted = await insertCategoryRuleIfMissing(client, {
          ...
          note: `Batch 6 soap rule: ${def.display} -> ${resolvedCategory}`,
        });
...
function printSummary(stats) {
  const lines = [];
  lines.push("==================================================");
  lines.push(` INGREDIENT DICTIONARY SEED — BATCH 6 (soap/cleanser)  [${stats.mode.toUpperCase()}]`);
  lines.push("==================================================");
...
async function main() {
  ...
  if (args.help) {
    console.log("node scripts/seed_ingredient_dictionary_batch6.js [--dry-run|--commit] [--db-url <url>]");
    return;
  }
...
if (require.main === module) {
  main().catch((error) => {
    console.error(`Batch 6 seed failed: ${error.message}`);
    process.exitCode = 1;
  });
}
