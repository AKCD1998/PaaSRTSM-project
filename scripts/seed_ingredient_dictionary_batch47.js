...
/**
 * Ingredient Dictionary Seed — Batch 47
 * Non-statin LDL-lowering agents / bile acid sequestrants — 5 entries
 *
 * SCREENING DECISIONS (2026-06-26):
 *
 *   INCLUDED:
 *     ezetimibe, bempedoic acid, cholestyramine, colestipol, colesevelam
 *
 *   EXISTING ENTRY UPGRADE:
 *     ezetimibe already exists from an early batch with English-only vocabulary.
 *     This batch adds Thai synonyms and updates display/indication/class text.
 *
 *   SALT / FORM RULES:
 *     colestipol hydrochloride / colestipol HCl   → colestipol
 *     colesevelam hydrochloride / colesevelam HCl → colesevelam
 *     cholestyramine resin                        → cholestyramine
 *       (resin / hydrochloride forms represent the same active binder moiety)
 *
 *   EXCLUDED TERM:
 *     cholestyramine chloride
 *       Not a standard marketed label term in the sources reviewed; avoid adding
 *       unsupported chemical-form text that could create future conflicts.
 *
 *   DEFERRED / EXCLUDED:
 *     colestimide — Japan-specific / 0 local hits; defer
 *     colextran   — insufficient current market evidence / 0 local hits; exclude
 *
 *   ALREADY MODELED ELSEWHERE:
 *     omega-3 / n-3 fatty acid class terms are already represented under the
 *     existing fish oil / omega-3 entry from batch 5. Not a new parent entry.
 *     niacin / nicotinic acid already exists from batch 4. Not expanded here
 *     because the old entry likely needs cleanup before broader synonym growth.
 *
 * Total: 5 entries, 19 synonyms (Thai: 8 / English: 11)
 *
 * Default --dry-run; --commit persists.
 *   node scripts/seed_ingredient_dictionary_batch47.js [--dry-run|--commit] [--db-url <url>]
 */
...
const SOURCE = "seed_dictionary_batch_47";

const INGREDIENTS = [
  {
    canonical: "ezetimibe",
    display: "Ezetimibe (เอเซทิไมบ์ / เอเซทิมิเบ)",
    drugClass: "Cholesterol absorption inhibitor (lipid-lowering agent)",
    indications: [
      "Hypercholesterolaemia / LDL-C reduction (Rx)",
      "Familial hypercholesterolaemia (Rx)",
      "Adjunct lipid-lowering therapy (Rx)",
    ],
  ...
  {
    canonical: "bempedoic acid",
    display: "Bempedoic Acid (กรดเบมเพโดอิก)",
    drugClass: "ATP-citrate lyase inhibitor (lipid-lowering agent)",
    indications: [
      "Hypercholesterolaemia / LDL-C reduction (Rx)",
      "Familial hypercholesterolaemia (Rx)",
  ...
  {
    canonical: "cholestyramine",
    display: "Cholestyramine (โคเลสไทรามีน)",
    drugClass: "Bile acid sequestrant / anion-exchange resin (lipid-lowering agent)",
    indications: [
      "Hypercholesterolaemia / LDL-C reduction (Rx)",
      "Bile acid sequestrant therapy (Rx)",
      "Adjunct lipid-lowering therapy (Rx)",
    ],
    preferredCategory: "ยาแผนปัจจุบัน",
    synonyms: [
  ...
  {
    canonical: "colestipol",
    display: "Colestipol (โคเลสติพอล)",
    drugClass: "Bile acid sequestrant / anion-exchange resin (lipid-lowering agent)",
    indications: [
      "Hypercholesterolaemia / LDL-C reduction (Rx)",
      "Bile acid sequestrant therapy (Rx)",
  ...
  {
    canonical: "colesevelam",
    display: "Colesevelam (โคเลเซเวลัม)",
    drugClass: "Bile acid sequestrant polymer (lipid-lowering agent)",
    indications: [
      "Hypercholesterolaemia / LDL-C reduction (Rx)",
      "Bile acid sequestrant therapy (Rx)",
      "Adjunct lipid-lowering therapy (Rx)",
    ...
    for (const def of INGREDIENTS) {
      ...
      if (resolvedCategory) {
        const ruleInserted = await insertCategoryRuleIfMissing(client, {
          ...
          note: `Batch 47 rule: ${def.canonical} -> ${resolvedCategory}`,
        });
...
function printSummary(stats) {
  const lines = [];
  lines.push("==================================================");
  lines.push(` INGREDIENT DICTIONARY SEED — BATCH 47 (Non-statin LDL-lowering agents)  [${stats.mode.toUpperCase()}]`);
  lines.push("==================================================");
...
async function main() {
  ...
  if (args.help) { console.log("node scripts/seed_ingredient_dictionary_batch47.js [--dry-run|--commit] [--db-url <url>]"); return; }
...
if (require.main === module) {
  main().catch((err) => { console.error(`Batch 47 seed failed: ${err.message}`); process.exitCode = 1; });
}
