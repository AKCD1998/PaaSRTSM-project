...
/**
 * Ingredient Dictionary Seed — Batch 37
 * รางจืด / Thunbergia laurifolia (Rang Chuet / Laurel clock vine)
 *
 * Single ingredient, 17 synonyms (Thai: 9 / English+scientific: 8).
 *
 * NOTES:
 *   "รางจืด" literally = "make flavours fade" → detoxification / neutralisation.
 *   Primary traditional Thai use: alcohol detox, antidote for food/plant poisons,
 *   hangover remedy. Modern research also shows antioxidant and anti-diabetic effects.
 *
 *   Product-form synonyms included (ชารางจืด, ยาชงรางจืด, รางจืดแคปซูล,
 *   ผงรางจืด) because they appear verbatim in product name fields and help
 *   the backfill scanner surface these products for proposal.
 *
 *   "Thunbergia laurifolia Lindl." — Lindley author abbreviation appears on some
 *   scientific / pharmacopoeia label declarations; included as distinct synonym.
 *
 *   "blue trumpet vine" and "laurel clock vine" are English horticultural names
 *   that appear on imported botanical extract labels.
 *
 * Default --dry-run; --commit persists.
 *   node scripts/seed_ingredient_dictionary_batch37.js [--dry-run|--commit] [--db-url <url>]
 */
...
const SOURCE = "seed_dictionary_batch_37";

const INGREDIENTS = [
  {
    canonical: "thunbergia laurifolia",
    display: "Rang Chuet / Laurel Clock Vine (รางจืด / Thunbergia laurifolia)",
    drugClass: "Herbal detoxifier / Antidote (Thunbergia laurifolia / Rang Chuet)",
    indications: [
      "Detoxification (herbal)",
      "Antidote / Poison neutralisation (traditional Thai)",
      "Hangover / Alcohol detoxification (herbal)",
    ],
    preferredCategory: "ยาสมุนไพร",
    synonyms: [
      // ── Thai names (leaf, extract, product-form variants) ─────────────────
      "รางจืด",
      "ใบรางจืด",
      "สารสกัดรางจืด",
      "รางจืดสกัด",
      ...
      "ยาชงรางจืด",
      "รางจืดแคปซูล",
      // ── Botanical (with and without Lindley author abbreviation) ──────────
      "thunbergia laurifolia",
      "thunbergia laurifolia lindl.",
      "thunbergia laurifolia extract",
      "thunbergia laurifolia leaf extract",
      // ── English romanised / common names ──────────────────────────────────
      "rang chuet",
      "rang jued",
      "laurel clock vine",
      "blue trumpet vine",
    ...
    for (const def of INGREDIENTS) {
      ...
      if (resolvedCategory) {
        const ruleInserted = await insertCategoryRuleIfMissing(client, {
          ...
          note: `Batch 37 rule: ${def.canonical} -> ${resolvedCategory}`,
        });
...
function printSummary(stats) {
  const lines = [];
  lines.push("==================================================");
  lines.push(` INGREDIENT DICTIONARY SEED — BATCH 37 (รางจืด / Thunbergia)  [${stats.mode.toUpperCase()}]`);
  lines.push("==================================================");
...
async function main() {
  ...
  if (args.help) { console.log("node scripts/seed_ingredient_dictionary_batch37.js [--dry-run|--commit] [--db-url <url>]"); return; }
...
if (require.main === module) {
  main().catch((err) => { console.error(`Batch 37 seed failed: ${err.message}`); process.exitCode = 1; });
}
