...
/**
 * Ingredient Dictionary Seed — Batch 46
 * PCSK9-targeted LDL-lowering agents — 3 entries
 *
 ...
 *
 *   SALT / FORM RULE:
 *     inclisiran sodium / inclisiran sodium salt  → inclisiran
 *       (same active moiety; LEQVIO label states 284 mg inclisiran is present
 *        as 300 mg inclisiran sodium salt)
 *
 *   DEFERRED (1 drug):
 *     lerodalcibep-liga / lerodalcibep
 *       FDA-approved in late 2025, but too new for this dictionary pass and
 *       there are 0 direct local catalog hits as of 2026-06-26. Revisit when
 *       it appears in Thai ordering data or broader market evidence is needed.
 *
 *   EXCLUDED (1 drug):
 *     bococizumab
 *       Development program discontinued by Pfizer in 2016; not an active
 *       marketed therapy for routine pharmacy ordering. Do not seed.
 *
 * Total: 3 entries, 7 synonyms (Thai: 3 / English: 4)
 *
 * Default --dry-run; --commit persists.
 *   node scripts/seed_ingredient_dictionary_batch46.js [--dry-run|--commit] [--db-url <url>]
 */
...
const SOURCE = "seed_dictionary_batch_46";

const INGREDIENTS = [
  {
    canonical: "alirocumab",
    display: "Alirocumab (อาลิโรคูแมบ)",
    drugClass: "PCSK9 inhibitor monoclonal antibody (lipid-lowering agent)",
    indications: [
  ...
  },
  {
    ...
    display: "Evolocumab (อีโวโลคูแมบ)",
    drugClass: "PCSK9 inhibitor monoclonal antibody (lipid-lowering agent)",
    indications: [
      "Hypercholesterolaemia / LDL-C reduction (Rx)",
      "Familial hypercholesterolaemia (Rx)",
  ...
  {
    canonical: "inclisiran",
    display: "Inclisiran (อินคลิซิแรน)",
    drugClass: "PCSK9-directed small interfering RNA (siRNA) lipid-lowering agent",
    indications: [
      "Hypercholesterolaemia / LDL-C reduction (Rx)",
      "Familial hypercholesterolaemia (Rx)",
    ...
    for (const def of INGREDIENTS) {
      ...
      if (resolvedCategory) {
        const ruleInserted = await insertCategoryRuleIfMissing(client, {
          ...
          note: `Batch 46 rule: ${def.canonical} -> ${resolvedCategory}`,
        });
...
function printSummary(stats) {
  const lines = [];
  lines.push("==================================================");
  lines.push(` INGREDIENT DICTIONARY SEED — BATCH 46 (PCSK9-targeted agents)  [${stats.mode.toUpperCase()}]`);
  lines.push("==================================================");
...
async function main() {
  ...
  if (args.help) { console.log("node scripts/seed_ingredient_dictionary_batch46.js [--dry-run|--commit] [--db-url <url>]"); return; }
...
if (require.main === module) {
  main().catch((err) => { console.error(`Batch 46 seed failed: ${err.message}`); process.exitCode = 1; });
}
