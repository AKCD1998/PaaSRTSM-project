...
/**
 * Ingredient Dictionary Seed — Batch 9 (Set 9: Collagen — synonym extension).
 *
 * The collagen ingredient was first seeded in batch 5 (canonical='collagen',
 * display='Collagen', preferredCategory='วิตามิน'). This batch EXTENDS it with
 * additional synonyms requested per the Set 9 curated list:
 *
 *   New synonyms added by this batch (batch-5 synonyms already in DB skip):
 *     beef collagen, eggshell collagen, type v collagen, type x collagen,
 *     uc-ii, verisol, fortigel, fortibone, bodybalance, peptan, naticol,
 *     collagen dipeptide, low molecular weight collagen
 *
 *   Synonyms already seeded by batch 5 (will skip gracefully):
 *     collagen, collagen peptide, collagen peptides, hydrolyzed collagen,
 *     hydrolysed collagen, collagen hydrolysate, marine collagen, fish collagen,
 *     bovine collagen, porcine collagen, chicken collagen, type i collagen,
 *     type ii collagen, type iii collagen, collagen tripeptide
 *
 * NOTE: "uc-ii" was previously listed in EXCLUDED_ABBREVIATIONS in batch 5
 * (alongside Zn/EPA/DHA etc.). It is included here per explicit user request
 * on 2026-06-16. UC-II is a branded undenatured type-II collagen ingredient
 * (InterHealth/Lonza) that appears on product labels — distinct from pure
 * biochem abbreviations.
 *
 * Branded collagen ingredient names (Verisol/FortiGel/Fortibone/BodyBalance/
 * Peptan/Naticol) are included because they are INGREDIENT trade names that
 * appear on product labels (unlike manufacturer/distributor brand tokens such
 * as tman/siam/biopharm which are irrelevant to ingredient identity).
 *
 * preferredCategory is kept as "วิตามิน" to match batch 5.
 * Same safe/idempotent pattern as batches 1-8. Default --dry-run; --commit persists.
 *   node scripts/seed_ingredient_dictionary_batch9.js [--dry-run|--commit] [--db-url <url>]
 */
...
const SOURCE = "seed_dictionary_batch_9";
const VITAMIN = "วิตามิน";

const MODELING_NOTES = [
  "Extends the existing batch-5 'collagen' ingredient with additional synonyms only; ingredient row is skipped (already exists).",
  '"uc-ii" was excluded in batch 5 (EXCLUDED_ABBREVIATIONS) but is included here per explicit user request (2026-06-16) — it is a branded ingredient name (undenatured type-II collagen) that appears on product labels.',
  "Branded ingredient names (Verisol, FortiGel, Fortibone, BodyBalance, Peptan, Naticol) are ingredient trade names on product labels, not manufacturer/distributor tokens — appropriate to seed.",
  "Synonyms already seeded by batch 5 will be gracefully skipped (global-unique synonym check).",
  "English-only synonyms; matcher is Latin-only, so Thai synonyms would not drive matching yet.",
];
...
const INGREDIENTS = [
  {
    canonical: "collagen",
    display: "Collagen",
    drugClass: "Supplement (Collagen)",
    indications: ["Skin/Joint supplement"],
    preferredCategory: VITAMIN,
    synonyms: [
      // ── Already seeded by batch 5 (will skip) ─────────────────────────────
      "collagen",
      "collagen peptide",
      "collagen peptides",
      "hydrolyzed collagen",
      ...
      "type v collagen",
      "type x collagen",
      // Branded / proprietary ingredient names
      "uc-ii",
      "uc ii",
      "verisol",
      "fortigel",
    ...
    for (const def of INGREDIENTS) {
      ...
      if (resolvedCategory) {
        const ruleInserted = await insertCategoryRuleIfMissing(client, {
          ...
          note: `Batch 9 collagen extension rule: ${def.display} -> ${resolvedCategory}`,
        });
        ...
        stats.uncertainCategoryMappings.push({
          ...
          reason: def.uncertainReason || `category "${def.preferredCategory}" not found`,
        });
      }
    }
...
function printSummary(stats) {
  const lines = [];
  lines.push("==================================================");
  lines.push(` INGREDIENT DICTIONARY SEED — BATCH 9 (collagen synonym extension)  [${stats.mode.toUpperCase()}]`);
  lines.push("==================================================");
...
async function main() {
  ...
  if (args.help) {
    console.log("node scripts/seed_ingredient_dictionary_batch9.js [--dry-run|--commit] [--db-url <url>]");
    return;
  }
...
if (require.main === module) {
  main().catch((error) => {
    console.error(`Batch 9 seed failed: ${error.message}`);
    process.exitCode = 1;
  });
}
