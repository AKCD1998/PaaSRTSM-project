...
/**
 * Ingredient Dictionary Seed — Batch 21 (ว่านหางจระเข้ / Aloe Vera).
 *
 * Single ingredient family. Covers the leaf gel, juice, and extract forms of
 * Aloe vera (Aloe barbadensis Miller). NOTE: This covers ALOE GEL only — not
 * aloe latex (the anthraquinone laxative from the leaf skin), which is a
 * separate compound. Do not merge with senna or other stimulant laxatives.
 *
 * Trimmed from user's list:
 *   "Aloe barbadensis Mill."     — botanical author abbreviation; not a label
 *                                  token. "Aloe barbadensis" (without author) covers it.
 *   "Aloe vera (L.) Burm. f."   — full botanical authority notation; never
 *                                  appears on pharmacy product labels.
 *
 * Added vs user's list:
 *   "ว่านหางจระเข้สด"            — fresh/raw form designation used on some labels
 *   "aloe"                       — shortened name on some imported topical labels
 *   "aloe vera inner leaf gel"   — common label term distinguishing inner gel
 *                                  from whole-leaf preparations
 *   "aloe barbadensis leaf gel"  — INCI name (International Nomenclature of
 *   "aloe barbadensis leaf juice"  Cosmetic Ingredients); all three INCI forms
 *   "aloe barbadensis leaf extract" appear on imported cosmetic ingredient lists
 *
 * Thai spelling note: both "ว่านหางจระเข้" and "ว่านหางจรเข้" (without สระ อา
 * in จระ) are in active use on Thai product labels — both are included.
 *
 * Thai synonyms (6): ว่านหางจระเข้, ว่านหางจรเข้, เจลว่านหางจระเข้,
 *                    สารสกัดว่านหางจระเข้, น้ำว่านหางจระเข้, ว่านหางจระเข้สด
 * English/scientific (11): aloe vera, aloe, aloe vera gel, aloe vera extract,
 *   aloe vera leaf extract, aloe vera juice, aloe vera inner leaf gel,
 *   aloe barbadensis, aloe barbadensis leaf gel, aloe barbadensis leaf juice,
 *   aloe barbadensis leaf extract
 *
 * preferredCategory = "ยาสมุนไพร" — skipped gracefully if not confirmed in DB.
 *
 * Default --dry-run; --commit persists.
 *   node scripts/seed_ingredient_dictionary_batch21.js [--dry-run|--commit] [--db-url <url>]
 */
...
const SOURCE = "seed_dictionary_batch_21";
const HERB_CATEGORY = "ยาสมุนไพร";

const MODELING_NOTES = [
  "Batch 21 = ว่านหางจระเข้ / Aloe Vera — leaf gel, juice, and extract forms unified.",
  "Covers ALOE GEL (inner leaf) only — NOT aloe latex (leaf skin anthraquinone laxative). Do not merge with senna.",
  "Both Thai spellings included: 'ว่านหางจระเข้' and 'ว่านหางจรเข้' — both appear on Thai product labels.",
  "'Aloe barbadensis Mill.' excluded: botanical author abbreviation, not a product label token.",
  "'Aloe vera (L.) Burm. f.' excluded: full botanical authority notation, never on pharmacy labels.",
  "'ว่านหางจระเข้สด' added: fresh/raw designation used on some Thai product labels.",
  "'aloe' added: shortened form on some imported topical labels.",
  "'aloe vera inner leaf gel' added: common label term distinguishing inner gel from whole-leaf.",
  "INCI names added: 'aloe barbadensis leaf gel/juice/extract' — appear on imported cosmetic ingredient lists.",
...
const INGREDIENTS = [
  {
    canonical: "aloe vera",
    display: "Aloe Vera (ว่านหางจระเข้)",
    drugClass: "Herbal extract (Mucilaginous / Skin-soothing)",
    indications: ["Minor burns / Sunburn relief (topical)", "Wound healing / Skin soothing (topical)", "Skin moisturizer / Emollient"],
    preferredCategory: HERB_CATEGORY,
    synonyms: [
      // ── Thai names (both spellings) ────────────────────────────────────────
      "ว่านหางจระเข้",
      "ว่านหางจรเข้",
      "เจลว่านหางจระเข้",
      ...
      // ── Botanical name ────────────────────────────────────────────────────
      "aloe barbadensis",
      // ── INCI names (cosmetic ingredient lists) ────────────────────────────
    ...
    for (const def of INGREDIENTS) {
      ...
      if (resolvedCategory) {
        const ruleInserted = await insertCategoryRuleIfMissing(client, {
          ...
          note: `Batch 21 rule: ${def.display} -> ${resolvedCategory}`,
        });
...
function printSummary(stats) {
  const lines = [];
  lines.push("==================================================");
  lines.push(` INGREDIENT DICTIONARY SEED — BATCH 21 (ว่านหางจระเข้ / Aloe Vera)  [${stats.mode.toUpperCase()}]`);
  lines.push("==================================================");
...
async function main() {
  ...
  if (args.help) {
    console.log("node scripts/seed_ingredient_dictionary_batch21.js [--dry-run|--commit] [--db-url <url>]");
    return;
  }
...
if (require.main === module) {
  main().catch((error) => {
    console.error(`Batch 21 seed failed: ${error.message}`);
    process.exitCode = 1;
  });
}
