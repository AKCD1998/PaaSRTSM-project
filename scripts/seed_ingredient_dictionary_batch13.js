...
/**
 * Ingredient Dictionary Seed — Batch 13 (Immune + Liver + Men's Health families).
 *
 * Part of Knowledge Layer v1 — Ingredient Family Dictionary for Thai pharmacy.
 *
 * IMMUNE SYSTEM (3 new families — VITAMIN_C seeded batch 3, ZINC seeded batch 5):
 *   ELDERBERRY, ECHINACEA, BETA_GLUCAN
 *
 * LIVER (1 new family — NAC/acetylcysteine already seeded in earlier batch):
 *   SILYMARIN
 *
 * PROSTATE / MEN (2 families):
 *   SAW_PALMETTO, LYCOPENE
 *
 * Already seeded — skipped:
 *   VITAMIN_C (batch 3), ZINC (batch 5), NAC/acetylcysteine (earlier batch)
 *
 * Abbreviation decisions:
 *   "β glucan" — β (U+03B2) is stripped by normalizeLatin to nothing, leaving
 *     only "glucan" as a standalone token — false-positive risk too high; use
 *     spelled-out "beta glucan" forms instead.
 *   "lycomato" / "lyc o mato" — LYC-O-MATO branded lycopene extract (LycoRed);
 *     kept as distinctive product label terms.
 *
 * All 6 families → preferredCategory = "วิตามิน".
 *
 * Same safe/idempotent pattern as batches 1-12. Default --dry-run; --commit persists.
 *   node scripts/seed_ingredient_dictionary_batch13.js [--dry-run|--commit] [--db-url <url>]
 */
...
const SOURCE = "seed_dictionary_batch_13";
const VITAMIN = "วิตามิน";

const MODELING_NOTES = [
  "Batch 13 = KL-v1 Immune + Liver + Men's Health: 6 families (ELDERBERRY, ECHINACEA, BETA_GLUCAN, SILYMARIN, SAW_PALMETTO, LYCOPENE).",
  "VITAMIN_C (batch 3), ZINC (batch 5), NAC/acetylcysteine (earlier batch) already seeded — skipped.",
  '"β glucan" excluded: β stripped by normalizeLatin to nothing, leaving bare "glucan" — false-positive risk. Spelled-out "beta glucan" forms used instead.',
  '"lycomato" and "lyc o mato" kept: LYC-O-MATO is a branded lycopene extract (LycoRed) that appears on product labels.',
...
const INGREDIENTS = [
  // ── IMMUNE SYSTEM ─────────────────────────────────────────────────────────
  {
    ...
    display: "Elderberry",
    drugClass: "Herbal extract (Immune)",
  ...
  {
    canonical: "beta glucan",
    display: "Beta-Glucan",
    drugClass: "Polysaccharide (Immune)",
    indications: ["Immune support"],
    preferredCategory: VITAMIN,
    synonyms: [
      "beta glucan",
      ...
      // "β glucan" excluded: β stripped by normalizeLatin, leaving bare "glucan"
    ],
  },
  // ── LIVER ─────────────────────────────────────────────────────────────────
  {
    canonical: "silymarin",
    display: "Silymarin (Milk Thistle)",
    drugClass: "Herbal extract (Liver)",
  ...
  // ── PROSTATE / MEN ────────────────────────────────────────────────────────
  {
    ...
    drugClass: "Herbal extract (Prostate/Men's health)",
    indications: ["Prostate health"],
    preferredCategory: VITAMIN,
    synonyms: [
      "saw palmetto",
  ...
  },
  {
    canonical: "lycopene",
    display: "Lycopene",
    drugClass: "Carotenoid (Antioxidant/Men's health)",
    indications: ["Prostate health", "Antioxidant"],
    preferredCategory: VITAMIN,
    synonyms: [
      "lycopene",
      "tomato extract",
    ...
    for (const def of INGREDIENTS) {
      ...
      if (resolvedCategory) {
        const ruleInserted = await insertCategoryRuleIfMissing(client, {
          ...
          note: `Batch 13 KL-v1 rule: ${def.display} -> ${resolvedCategory}`,
        });
...
function printSummary(stats) {
  const lines = [];
  lines.push("==================================================");
  lines.push(` INGREDIENT DICTIONARY SEED — BATCH 13 (Immune + Liver + Men's Health)  [${stats.mode.toUpperCase()}]`);
  lines.push("==================================================");
...
async function main() {
  ...
  if (args.help) {
    console.log("node scripts/seed_ingredient_dictionary_batch13.js [--dry-run|--commit] [--db-url <url>]");
    return;
  }
...
if (require.main === module) {
  main().catch((error) => {
    console.error(`Batch 13 seed failed: ${error.message}`);
    process.exitCode = 1;
  });
}
