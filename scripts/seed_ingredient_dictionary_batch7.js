...
/**
 * Ingredient Dictionary Seed — Batch 7 (Set 7: Alcohol / Antiseptic).
 *
 * Models a single broad "alcohol" category-ingredient covering ethyl alcohol,
 * isopropyl alcohol, hand sanitizers / rubs, antiseptic & disinfectant solutions
 * and alcohol-prep pads/swabs. Like soap (batch 6) this is a product-type/shelf
 * term rather than a true single active ingredient — seeded so the review queue
 * can propose the alcohol shelf for these products.
 *
 * Decisions baked in (per user, 2026-06-16):
 *   - Short abbreviations "ipa" and "abhr" ARE included (against the batch 4/5
 *     precedent of excluding short abbreviations) — "ipa" is high-value labeling
 *     for isopropyl alcohol. They match whole-token only; small false-positive
 *     risk is acceptable because every row is status='proposed' for review.
 *   - preferredCategory = "แอลกอฮอล์" (alcohol shelf).
 *
 * Modeling caveat: the bare "alcohol" synonym also matches EXCIPIENT alcohols in
 * creams/lotions (cetyl/stearyl/benzyl/cetostearyl alcohol). Those are false
 * positives for an antiseptic shelf, but they land as 'proposed' so pharmacist
 * review filters them out.
 *
 * Matching note: the production matcher (backfill_product_ingredient_proposals.js
 * -> scanProduct) uses coverage.normalizeLatin + whole-token substring matching.
 * normalizeLatin collapses non-alphanumerics (hyphens, %, etc.) to spaces, so
 * "2-propanol" -> "2 propanol" and "ETHANOL 70%" -> "ethanol 70".
 *
 * English-only synonyms; matcher is Latin-only so Thai would not drive matching.
 * Same safe/idempotent pattern as batches 1-6. Default --dry-run; --commit persists.
 *   node scripts/seed_ingredient_dictionary_batch7.js [--dry-run|--commit] [--db-url <url>]
 */
...
const SOURCE = "seed_dictionary_batch_7";
const ALCOHOL = "แอลกอฮอล์";

const MODELING_NOTES = [
  '"alcohol" is a product-type/shelf term, not a true single active ingredient — seeded so the review queue can propose the alcohol shelf.',
  'Short abbreviations "ipa" and "abhr" are included per user decision (high-value labeling for isopropyl alcohol / alcohol-based hand rub); whole-token match only.',
  'Bare "alcohol" also matches excipient alcohols (cetyl/stearyl/benzyl/cetostearyl alcohol) in creams — false positives for an antiseptic shelf, caught at review since all rows are proposed.',
  'normalizeLatin collapses hyphens/percent/etc to spaces: "2-propanol" -> "2 propanol", "ETHANOL 70%" -> "ethanol 70".',
...
const INGREDIENTS = [
  {
    canonical: "alcohol",
    display: "Alcohol / Antiseptic",
    drugClass: "Antiseptic / Disinfectant",
    indications: ["Antisepsis/Disinfection"],
    preferredCategory: ALCOHOL,
    uncertainReason: 'Alcohol shelf "แอลกอฮอล์" not present in confirmed categories.',
    synonyms: [
      "alcohol",
      "ethyl alcohol",
      "ethanol",
    ...
    for (const def of INGREDIENTS) {
      ...
      if (resolvedCategory) {
        const ruleInserted = await insertCategoryRuleIfMissing(client, {
          ...
          note: `Batch 7 alcohol rule: ${def.display} -> ${resolvedCategory}`,
        });
...
function printSummary(stats) {
  const lines = [];
  lines.push("==================================================");
  lines.push(` INGREDIENT DICTIONARY SEED — BATCH 7 (alcohol/antiseptic)  [${stats.mode.toUpperCase()}]`);
  lines.push("==================================================");
...
async function main() {
  ...
  if (args.help) {
    console.log("node scripts/seed_ingredient_dictionary_batch7.js [--dry-run|--commit] [--db-url <url>]");
    return;
  }
...
if (require.main === module) {
  main().catch((error) => {
    console.error(`Batch 7 seed failed: ${error.message}`);
    process.exitCode = 1;
  });
}
