...
/**
 * Ingredient Dictionary Seed — Batch 36
 * เพชรสังฆาต / Cissus quadrangularis (Veldt grape / Hadjod)
 *
 * Single ingredient, 15 synonyms (Thai: 5 / English+scientific: 10).
 *
 * NOTES:
 *   canonical "cissus quadrangularis" chosen; "cissus quadrangularis l." included
 *   as distinct synonym because the author abbreviation "L." (Linnaeus) appears
 *   verbatim on some product label / ingredient declarations.
 *
 *   Uses:
 *     - Traditional Thai / Ayurvedic: bone fracture healing, joint pain
 *     - "hadjod" = Ayurvedic name meaning "bone setter"
 *     - Modern supplement market: joint support, anti-inflammatory, weight mgmt
 *
 * Default --dry-run; --commit persists.
 *   node scripts/seed_ingredient_dictionary_batch36.js [--dry-run|--commit] [--db-url <url>]
 */
...
const SOURCE = "seed_dictionary_batch_36";

const INGREDIENTS = [
  {
    canonical: "cissus quadrangularis",
    display: "Veldt Grape / Hadjod (เพชรสังฆาต / Cissus quadrangularis)",
    drugClass: "Herbal bone/joint tonic / Anti-inflammatory (Cissus quadrangularis / Veldt grape)",
    indications: [
      "Bone healing / Fracture support (herbal)",
      "Joint health / Osteoarthritis support (herbal)",
    ...
    synonyms: [
      ...
      "เพชรสังฆาตสกัด",
      "สมุนไพรเพชรสังฆาต",
      // ── Botanical (with and without Linnaeus author abbreviation) ─────────
...
// ── env / db helpers (shared pattern across all batch scripts) ────────────────
    ...
    for (const def of INGREDIENTS) {
      ...
      if (resolvedCategory) {
        const ruleInserted = await insertCategoryRuleIfMissing(client, {
          ...
          note: `Batch 36 rule: ${def.canonical} -> ${resolvedCategory}`,
        });
...
function printSummary(stats) {
  const lines = [];
  lines.push("==================================================");
  lines.push(` INGREDIENT DICTIONARY SEED — BATCH 36 (เพชรสังฆาต / Cissus)  [${stats.mode.toUpperCase()}]`);
  lines.push("==================================================");
...
async function main() {
  ...
  if (args.help) { console.log("node scripts/seed_ingredient_dictionary_batch36.js [--dry-run|--commit] [--db-url <url>]"); return; }
...
if (require.main === module) {
  main().catch((err) => { console.error(`Batch 36 seed failed: ${err.message}`); process.exitCode = 1; });
}
