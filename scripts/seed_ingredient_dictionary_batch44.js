...
/**
 * Ingredient Dictionary Seed — Batch 44
 * Fibrates (PPAR-α agonists / lipid-lowering agents) — 6 entries
 *
 * SCREENING DECISIONS (2026-06-26):
 *
 *   INCLUDED:
 *     bezafibrate, ciprofibrate, clofibrate, fenofibrate, gemfibrozil, pemafibrate
 *
 *   SALT / COMPLEX FORMS → synonym of parent (same active moiety):
 *     aluminium clofibrate / อะลูมิเนียมโคลไฟเบรต  → clofibrate
 *     choline fenofibrate  / โคลีนฟีโนไฟเบรต       → fenofibrate
 *     fenofibric acid      / ฟีโนไฟบริกแอซิด        → fenofibrate
 *       (fenofibric acid is the active hydrolysis product; choline fenofibrate
 *        and micronized forms all appear on Thai dispensed-product labels)
 *
 *   EXCLUDED (4 drugs):
 *     etofibrate   — European niche (Germany/Italy), not present in Thai pharmacy
 *     simfibrate   — Japan-specific; unlikely in Thai pharmacy market
 *     ronifibrate  — historical / essentially discontinued; near-zero availability
 *     clofibride   — very niche, likely discontinued; no Thai name provided
 *
 *   NOTE ON CLOFIBRATE:
 *     Clofibrate is NOT formally withdrawn (unlike cerivastatin). It was
 *     voluntarily discontinued by most manufacturers due to poorer risk-benefit
 *     profile vs. newer fibrates. Legacy products may remain in ordering DB.
 *     Seed as ACTIVE ingredient with standard fibrate indications.
 *
 * Total: 6 entries, 21 synonyms (Thai: 9 / English: 12)
 *
 * Default --dry-run; --commit persists.
 *   node scripts/seed_ingredient_dictionary_batch44.js [--dry-run|--commit] [--db-url <url>]
 */
...
const SOURCE = "seed_dictionary_batch_44";

const INGREDIENTS = [
  {
    canonical: "bezafibrate",
    display: "Bezafibrate (เบซาฟิเบรต)",
    drugClass: "PPAR-α agonist / Fibrate (lipid-lowering agent)",
    indications: [
      "Hypertriglyceridaemia / Dyslipidaemia (Rx)",
      "HDL cholesterol elevation (Rx)",
  ...
  {
    canonical: "ciprofibrate",
    display: "Ciprofibrate (ซิโปรไฟเบรต)",
    drugClass: "PPAR-α agonist / Fibrate (lipid-lowering agent)",
    indications: [
      "Hypertriglyceridaemia / Dyslipidaemia (Rx)",
      "HDL cholesterol elevation (Rx)",
  ...
  {
    canonical: "clofibrate",
    display: "Clofibrate (โคลไฟเบรต)",
    drugClass: "PPAR-α agonist / Fibrate (lipid-lowering agent)",
    indications: [
      "Hypertriglyceridaemia / Dyslipidaemia (Rx)",
      "HDL cholesterol elevation (Rx)",
    ...
    synonyms: [
      "โคลไฟเบรต",
      "clofibrate",
      // ── Salt / complex forms (same active moiety) ─────────────────────────
      "อะลูมิเนียมโคลไฟเบรต",
  ...
  },
  {
    ...
    display: "Fenofibrate (ฟีโนไฟเบรต)",
    drugClass: "PPAR-α agonist / Fibrate (lipid-lowering agent)",
    indications: [
      "Hypertriglyceridaemia / Dyslipidaemia (Rx)",
      "HDL cholesterol elevation (Rx)",
    ...
    synonyms: [
      "ฟีโนไฟเบรต",
      "fenofibrate",
      // ── Salt / active-metabolite forms ────────────────────────────────────
      "โคลีนฟีโนไฟเบรต",
      "choline fenofibrate",
      "ฟีโนไฟบริกแอซิด",
      "fenofibric acid",
      // ── Formulation variants (appear verbatim on Thai dispensed labels) ───
      "ฟีโนไฟเบรตไมโครไนซ์",
  ...
  },
  {
    ...
    display: "Gemfibrozil (เจมไฟโบรซิล)",
    drugClass: "PPAR-α agonist / Fibrate (lipid-lowering agent)",
    indications: [
      "Hypertriglyceridaemia / Dyslipidaemia (Rx)",
      "HDL cholesterol elevation (Rx)",
  ...
  {
    canonical: "pemafibrate",
    display: "Pemafibrate (เพมาฟิเบรต)",
    drugClass: "PPAR-α agonist / Fibrate (lipid-lowering agent)",
    indications: [
      "Hypertriglyceridaemia / Dyslipidaemia (Rx)",
      "HDL cholesterol elevation (Rx)",
    ...
    for (const def of INGREDIENTS) {
      ...
      if (resolvedCategory) {
        const ruleInserted = await insertCategoryRuleIfMissing(client, {
          ...
          note: `Batch 44 rule: ${def.canonical} -> ${resolvedCategory}`,
        });
...
function printSummary(stats) {
  const lines = [];
  lines.push("==================================================");
  lines.push(` INGREDIENT DICTIONARY SEED — BATCH 44 (Fibrates / PPAR-α agonists)  [${stats.mode.toUpperCase()}]`);
  lines.push("==================================================");
...
async function main() {
  ...
  if (args.help) { console.log("node scripts/seed_ingredient_dictionary_batch44.js [--dry-run|--commit] [--db-url <url>]"); return; }
...
if (require.main === module) {
  main().catch((err) => { console.error(`Batch 44 seed failed: ${err.message}`); process.exitCode = 1; });
}
